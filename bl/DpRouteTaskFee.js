...
function queryDpRouteTaskFeeDayStat(req,res,next){
    var params = req.params ;
    dpRouteTaskFeeDAO.getDpRouteTaskFeeDayStat(params,function(error,result){
        if (error) {
            logger.error(' queryDpRouteTaskFeeDayStat ' + error.message);
            throw sysError.InternalError(error.message,sysMsg.SYS_INTERNAL_ERROR_MSG);
        } else {
            logger.info(' queryDpRouteTaskFeeDayStat ' + 'success');
            resUtil.resetQueryRes(res,result,null);
            return next();
        }
    })
...
module.exports = {
    ...
    queryDpRouteTaskFeeDayStat : queryDpRouteTaskFeeDayStat,

...
function querySettleOuterCarList(req,res,next){
    var params = req.params ;
    settleOuterTruckDAO.getSettleOuterCarList(params,function(error,result){
        if (error) {
            logger.error(' getSettleOuterCarList ' + error.message);
            throw sysError.InternalError(error.message,sysMsg.SYS_INTERNAL_ERROR_MSG);
        } else {
            logger.info(' getSettleOuterCarList ' + 'success');
            resUtil.resetQueryRes(res,result,null);
            return next();
        }
    })
...
function querySettleOuterCarCount(req,res,next){
    var params = req.params ;
    settleOuterTruckDAO.getSettleOuterCarCount(params,function(error,result){
        if (error) {
            logger.error(' getSettleOuterCarCount ' + error.message);
            throw sysError.InternalError(error.message,sysMsg.SYS_INTERNAL_ERROR_MSG);
        } else {
            logger.info(' getSettleOuterCarCount ' + 'success');
            resUtil.resetQueryRes(res,result,null);
            return next();
        }
    })
...
function getSettleOuterCarCsv(req,res,next){
    var csvString = "";
    var header = "VIN"+ ',' +"品牌"+ ',' +"外协公司"+ ',' +"委托方"+ ','+"始发城市"+ ','+"装车地点"+ ','+"目的城市"+ ','+
        "经销商"+ ','+"指令时间"+ ','+"公里数(公里)"+ ','+"价格/公里"+ ','+"金额";
    csvString = header + '\r\n'+csvString;
    var params = req.params ;
    var parkObj = {};
    var fees = 0;
    settleOuterTruckDAO.getSettleOuterCarList(params,function(error,rows){
        if (error) {
            logger.error(' getSettleOuterTruckList ' + error.message);
            throw sysError.InternalError(error.message,sysMsg.SYS_INTERNAL_ERROR_MSG);
        } else {
            for(var i=0;i<rows.length;i++){
                ...
                csvString = csvString+parkObj.vin+","+parkObj.makeName+","+parkObj.companyName+","+
                    parkObj.eShortName+","+parkObj.routeStart+","+parkObj.addrName+","+parkObj.routeEnd+","+parkObj.rShortName+","+
                    parkObj.orderDate+","+parkObj.distance+","+parkObj.fee+","+parkObj.fees+'\r\n';
            }
            var csvBuffer = new Buffer(csvString,'utf8');
            res.set('content-type', 'application/csv');
            res.set('charset', 'utf8');
...
module.exports = {
    ...
    querySettleOuterCarCount : querySettleOuterCarCount,
    updateSettleOuterTruck : updateSettleOuterTruck,
    getSettleOuterTruckCsv : getSettleOuterTruckCsv,
    getSettleOuterCarCsv : getSettleOuterCarCsv
}

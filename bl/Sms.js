...
function sendPhoneSms(req,res,next){
    var params = req.params;
    var captcha = ""
    Seq().seq(function(){
        var that = this;
        ...
        userDAO.getUser(params,function(error,rows){
            if (error) {
                ...
                if(rows ==null || rows.length==0){
                    logger.warn(' sendPhoneSms ' +params.truckNum+ sysMsg.ADMIN_LOGIN_USER_UNREGISTERED);
                    resUtil.resetFailedRes(res,sysMsg.ADMIN_LOGIN_USER_UNREGISTERED);
                    return next();
                }else if(rows[0].status !=1){
                    logger.warn(' sendPhoneSms ' +params.truckNum+ sysMsg.SYS_AUTH_TOKEN_ERROR);
                    resUtil.resetFailedRes(res,sysMsg.SYS_AUTH_TOKEN_ERROR);
                    return next();
                }else{
                    that();
        ...
        var that = this;
        captcha = encrypt.getSmsRandomKey();
        oauthUtil.savePhoneCode({phone:params.mobile,code:captcha},function(error,result){
            if (error) {
                logger.error(' sendPhoneSms ' + error.message);
                resUtil.resetFailedRes(res,sysMsg.SYS_INTERNAL_ERROR_MSG);
                return next();
        ...
        smsDAO.sendSms({phone:params.mobile,captcha:captcha,templateId:smsConfig.smsOptions.signTemplateId},function (error,result) {
            if (error) {
                ...
                logger.info(' sendPhoneSms ' + 'success');
                resUtil.resetCreateRes(res,{insertId:1},null);
                return next();
            }
        })

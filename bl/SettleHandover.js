        ...
        if (error) {
            ...
            for(var i=0;i<rows.length;i++){
                ...
                csvString = csvString+parkObj.driveName+","+parkObj.truckNum+","+parkObj.operateType+","+parkObj.companyName+","+
                    ...
                    parkObj.distanceSalary+","+parkObj.reverseSalary+","+parkObj.salaryRatio+","+parkObj.level+'\r\n';
            }
            var csvBuffer = new Buffer(csvString,'utf8');
            res.set('content-type', 'application/csv');
            res.set('charset', 'utf8');
...
function getDriveSettleDetailCsv(req,res,next){
    ...
    var header = "调度编号"+ ','+"司机"+ ',' +"货车牌号"+ ',' +"计划执行时间"+ ',' +"VIN"+ ','+"委托方"+ ','+"品牌"+ ','
        ...
        +"经销商"+ ','+ "送入中转站 "+','+"到库"+','+"车型"+ ','+"产值率"+ ','+"预估公里数"+ ','+"预估单价"+ ','+"产值"+ ','+"二级产值";
    csvString = header + '\r\n'+csvString;
    var params = req.params ;
    var parkObj = {};
    settleHandoverDAO.getDriveSettleDetail(params,function(error,rows){
        if (error) {
            ...
            for(var i=0;i<rows.length;i++){
                ...
                if(rows[i].receive_flag == null){
                    parkObj.receiveFlag = "";
                }else{
                    parkObj.receiveFlag = rows[i].receive_flag;
                }
                ...
                csvString = csvString+parkObj.id+","+parkObj.driveName+","+parkObj.truckNum+","+parkObj.taskPlanDate+","+
                    ...
                    parkObj.shortName+","+parkObj.transferFlag+","+parkObj.receiveFlag+","+parkObj.sizeType+","+parkObj.outputRatio+","+parkObj.distance +","+parkObj.fee+","+ parkObj.output+","+
                    parkObj.twoOutput+ '\r\n';
            }
            var csvBuffer = new Buffer(csvString,'utf8');
            res.set('content-type', 'application/csv');

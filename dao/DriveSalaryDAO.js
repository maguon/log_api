...
function getDriveSalary(params,callback) {
    var query = " select ds.id,ds.month_date_id,ds.truck_id,ds.company_id,ds.user_id,ds.load_distance,ds.no_load_distance,ds.salary_ratio," +
        ...
        "               when drt.reverse_flag=0 and drt.truck_number=8 and drt.car_count>=9 and distance<500 then drt.distance*1.4 " +
        "               when drt.reverse_flag=0 and drt.truck_number=8 and drt.car_count>=9 and distance>=500 then drt.distance*1.2 " +
        "               end) distance_salary, " +

...
function getCityRouteDispatch(params,callback) {
    var query = " select "+ params.routeStartId +" ,ci.id as end_id,ci.city_name,cd.distance,cd.protect_fee,cd.id from city_info ci left join " +
        " (select cri.route_start_id as start_id, cri.route_end_id as end_id ,cri.distance,cri.protect_fee,cri.id from city_route_info cri where cri.route_start_id = " + params.routeStartId +
        " union " +
        " select cri2.route_end_id as start_id ,cri2.route_start_id as end_id ,cri2.distance,cri2.protect_fee,cri2.id from city_route_info cri2 where cri2.route_end_id = " + params.routeStartId +

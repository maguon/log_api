...
function addDriveSocialSecurity(params,callback){
    var query = "insert into drive_social_security (drive_id,drive_name,mobile,y_month,social_security_fee) " +
        " values ( ? , ? , ? , ? , ? ) ";
    var paramsArray=[],i=0;
...
function getDriveSocialSecurity(params,callback) {
    ...
    var paramsArray=[],i=0;
    if(params.socialSecurityId){
        paramsArray[i++] = params.socialSecurityId;
...
function updateDriveSocialSecurity(params,callback){
    var query = " update drive_social_security set social_security_fee = ? where id = ?" ;
    var paramsArray=[],i=0;
    paramsArray[i++]=params.socialSecurityFee;
    paramsArray[i++]=params.socialSecurityId;
    db.dbQuery(query,paramsArray,function(error,rows){
